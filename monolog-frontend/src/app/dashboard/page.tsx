...
import { AlertCircle, BarChart3, FileText, MessageSquare, Plus } from "lucide-react"
...
export default function DashboardPage() {
  ...
  const statCards = [
    { label: "Total Posts", value: stats?.totalPosts || "0", icon: FileText, trend: "Published articles" },
    { label: "Total Reads", value: stats?.totalViews || "0", icon: BarChart3, trend: "Unique views" },
    { label: "Pending Comments", value: stats?.pendingComments || "0", icon: MessageSquare, trend: "Review required" },
  ]
    ...
    <div className="p-4 md:p-8 space-y-8">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {statCards.map((stat) => {
          ...
          return (
            <div key={stat.label} className="rounded-2xl border border-border bg-white dark:bg-[#0F172A] p-6 shadow-level-1">
              ...
              <div className="space-y-1">
                <p className="text-3xl font-extrabold text-surface-on">{stat.value}</p>
